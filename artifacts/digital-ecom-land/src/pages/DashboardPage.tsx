...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Welcome Banner */}
      <div className="relative overflow-hidden rounded-2xl border border-primary/20 bg-card p-8 shadow-lg flex flex-col md:flex-row items-center gap-8 justify-between">
        <div className="absolute top-0 right-0 -translate-y-12 translate-x-12 opacity-10 pointer-events-none">
          <Compass className="size-64 text-primary" />
        </div>
        <div className="relative z-10 flex-1 space-y-4">
          <h2 className="text-2xl font-bold">Welcome back, Partner! 🚀</h2>
          <p className="text-muted-foreground max-w-xl">
            Watch our masterclass to learn the best strategies for running ads, 
            finding winning products, and maximizing your margins on Ecom Land.
          </p>
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <Button className="gap-2 shadow-[0_0_15px_rgba(229,169,60,0.3)]">
          ...
          <CardContent>
            {isLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground mt-1">Out of {stats?.totalOrders || 0} total</p>
              </>
            )}
          </CardContent>
        </Card>
      ...
      <div className="flex items-center gap-4">
        <Button asChild variant="outline" className="w-full h-16 text-lg gap-3">
          <Link href="/dashboard/products">
            <Compass className="size-5" /> Browse Catalog
          </Link>
        </Button>
        <Button asChild className="w-full h-16 text-lg gap-3 shadow-[0_0_20px_rgba(229,169,60,0.2)]">
          <Link href="/dashboard/orders">
            <PackagePlus className="size-5" /> Add New Order
          </Link>
        </Button>
      </div>
    </div>
